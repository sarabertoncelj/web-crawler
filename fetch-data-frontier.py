...
class MultiThreadScraper:
    def __init__(self, base_url_1, base_url_2, base_url_3, base_url_4):
        ...
        frontier_logs = dbfunctions.get_frontier(conn)
        for url in frontier_logs:
            self.frontier.put(url[0])
            #print(url[0])

        history_logs = dbfunctions.get_history(conn)
...
# from urlparse import urlparse  # Python 2
    ...
    def scrape_callback(self, response):
        ...
        html, target_url, http_status_code = response.result()
        # SSprint (target_url)
        if response.result() and (http_status_code == "200"):
            self.parse_site(conn, target_url)
            self.parse_page(conn, html, target_url, http_status_code)
            ...
            dbfunctions.create_new_page(conn, site_id, 'NULL', target_url, 'NULL', html_content_hash, http_status_code, accessed_time, target_url)
        else:
    ...
    def parse_site (self, conn, url):
        #print("PARSE SITE")
        domain = self.get_domain(url)
        if dbfunctions.get_site_id(conn, domain) == -1 :
            robots_url  = self.get_domain_url(url) + "robots.txt"
            ...
            sitemap_url = self.get_domain_url(url) + "sitemap.xml"
    ...
    def parse_page (self, conn, html_content, url, http_status_code):
        #print("PARSE PAGE")
        domain = self.get_domain(url)
        site_id = dbfunctions.get_site_id(conn, domain)
        if site_id == -1:
        ...
        canonical = self.is_canonical(html_content, url)
        if url != canonical:
            #print("Canonical URL")
            page_type_code = "DUPLICATE"
            html_content_hash = None
            id = dbfunctions.create_new_page(conn, site_id, page_type_code, url, 'NULL', html_content_hash, http_status_code, accessed_time, url)
            duplicate_id = dbfunctions.get_page_id(conn, url)
            dbfunctions.update_link(conn, duplicate_id, id)
        elif same_hash != -1:
            page_type_code = "DUPLICATE"
            html_content_hash = None
            id = dbfunctions.create_new_page(conn, site_id, page_type_code, url, 'NULL', html_content_hash, http_status_code, accessed_time, url)
            dbfunctions.update_link(conn, same_hash, id)
        else:
            ...
            id = dbfunctions.create_new_page(conn, site_id, page_type_code, url, html_content, html_content_hash, http_status_code, accessed_time, url)
